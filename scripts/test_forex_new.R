...
slfm_m50_D10 <- slfm_BART(Y_train, X_train, X_train, cutpoints, verbose = FALSE, D = 10, m = 50, nd = 1000, burn = 1500)
slfm_m50_D25 <- slfm_BART(Y_train, X_train, X_train, cutpoints, verbose = FALSE, D = 25, m = 50, nd = 1000, burn = 1500)
slfm_m10_D50 <- slfm_BART(Y_train, X_train, X_train, cutpoints, verbose = FALSE, D = 50, m = 10, nd = 1000, burn = 1500)

slfm2_m50_D10 <- slfm_BART2(Y_train, X_train, X_train, cutpoints, verbose = FALSE, D = 10, m = 50, nd = 1000, burn = 1500)
slfm2_m50_D25 <- slfm_BART2(Y_train, X_train, X_train, cutpoints, verbose = FALSE, D = 25, m = 50, nd = 1000, burn = 1500)
slfm2_m10_D50 <- slfm_BART2(Y_train, X_train, X_train, cutpoints, verbose = FALSE, D = 50, m = 10, nd = 1000, burn = 1500)

# Need to get the values of sigma_phi
sigma_phi_D10 <- rep(NA, times = ncol(Y_train))
sigma_phi_D25 <- rep(NA, times = ncol(Y_train))
sigma_phi_D50 <- rep(NA, times = ncol(Y_train))
for(k in 1:ncol(Y_train)){
  sigma_phi_D10[k] <- get_sigma_phi(Y_train[,k], target_prob = 0.8, df = 10)
  sigma_phi_D25[k] <- get_sigma_phi(Y_train[,k], target_prob = 0.8, df = 25)
  sigma_phi_D50[k] <- get_sigma_phi(Y_train[,k], target_prob = 0.8, df = 50)
}


slfm3_m50_D10 <- slfm_BART3(Y_train, X_train, X_train, cutpoints, sigma_phi = sigma_phi_D10, verbose = FALSE, D = 10, m = 50, nd = 1000, burn = 1500)
slfm3_m50_D25 <- slfm_BART3(Y_train, X_train, X_train, cutpoints, sigma_phi = sigma_phi_D25, verbose = FALSE, D = 25, m = 50, nd = 1000, burn = 1500)
slfm3_m10_D50 <- slfm_BART3(Y_train, X_train, X_train, cutpoints, sigma_phi = sigma_phi_D50, verbose = FALSE, D = 50, m = 10, nd = 1000, burn = 1500)


############
sep_fit <- sep_BART(Y_train, X_train, X_train, cutpoints, verbose = FALSE, nd = 1000, burn = 1500)
...
save(slfm_m50_D10, slfm2_m50_D10, slfm3_m50_D10, file = "~/Dropbox/Broderick_Group/bart_extensions/july16_forex_m50_D10.RData")
save(slfm_m50_D25, slfm2_m50_D25, slfm3_m50_D25, file = "~/Dropbox/Broderick_Group/bart_extensions/july16_forex_m50_D25.RData")
save(slfm_m10_D50, slfm2_m10_D50, slfm3_m10_D50, file = "~/Dropbox/Broderick_Group/bart_extensions/july16_forex_m10_D50.RData")
save(test_smse, file = "~/Dropbox/Broderick_Group/bart_extensions/july16_test_smse.RData")
...
#method_list <- c("slfm_m50_D10", "slfm2_m50_D10", "new_slmf2_m50_D10")
method_list <- c("slfm_m50_D10", "slfm2_m50_D10", "slfm_m50_D25", "slfm2_m50_D25", "slfm_m10_D50", "slfm2_m10_D50", "sep_fit")
test_smse <- data.frame("USD.CAD" = rep(NA, times = length(method_list)),
                        "USD.JPY" = rep(NA, times = length(method_list)),
                        "USD.AUD" = rep(NA, times = length(method_list)))
rownames(test_smse) <- method_list
