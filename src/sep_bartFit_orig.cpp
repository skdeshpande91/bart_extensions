                 ...
                 int burn = 250, int nd = 1000, int m = 200, double kappa = 2, double nu = 3, double var_prob = 0.9, bool verbose = false)
{
  ...
  if(verbose == true) Rcout << "created y_ptr, x_ptr, x_pred_ptr" << endl;
  ...
  if(verbose == true) Rcout << "Created xi" << endl;
  ...
  if(verbose == true) Rcpp::Rcout << "initialized pointers for residuals" << endl;
  ...
  arma::mat sigma_samples = arma::zeros<arma::mat>(q,nd); // just the square root of the diagonal elements of Sigma
  ...
  for(int iter = 0; iter < (nd + burn); iter++){
    ...
    // need to invert Omega to get Sigma...
    ...
    if(iter >= burn){
      for(size_t k = 0; k < q; k++){
        ...
        sigma_samples(k,iter-burn) = sqrt(Sigma_samples(k,k,iter-burn));
      }
  ...
  results["f_train_samples"] = train_samples;
  results["f_test_samples"] = test_samples;
