...
// 17 June 2019: track the outcome
double bd_uni(tree &x, const double &sigma, xinfo &xi, data_info &di, tree_prior_info &tree_pi, size_t k,RNG &gen)
{
  ...
  if(unif < PBx) {
    ...
    if((sl.n>=5) && (sr.n>=5)) { //kluge? [SKD]: why do we have this requirement??
      mu_posterior_uni(mu_bar_l, V_l, sigma, sl, di, tree_pi, k);
      mu_posterior_uni(mu_bar_r, V_r, sigma, sr, di, tree_pi, k);
      mu_posterior_uni(mu_bar_t, V_t, sigma, st, di, tree_pi, k);
...
// overloaded
double bd_slfm(tree &x, const arma::mat &Phi, const std::vector<double> &sigma, xinfo &xi, data_info &di, tree_prior_info &tree_pi, phi_prior_info &phi_pi, RNG &gen)
{
  tree::npv goodbots;  //nodes we could birth at (split on)
  double PBx = getpb(x,xi,tree_pi,goodbots);  //prob of a birth at x
  double unif = 0.0;
  ...
  if(unif < PBx){
    ...
    if((sl.n>=5) && (sr.n>=5)) { //kludge? [SKD]: why do we have this requirement??
      mu_posterior_slfm(mu_bar_l, V_l, Phi, sigma, sl, di, tree_pi, phi_pi);
      mu_posterior_slfm(mu_bar_r, V_r, Phi, sigma, sr, di, tree_pi, phi_pi);
      mu_posterior_slfm(mu_bar_t, V_t, Phi, sigma, st, di, tree_pi, phi_pi);
      ...
      //Rcpp::Rcout << "[bd]:   One of left and right child has fewer than 1 observations. Setting alpha = 0" << endl;
      alpha=0.0;
    }
    //Rcpp::Rcout << "  alpha = " << alpha;
    //--------------------------------------------------
