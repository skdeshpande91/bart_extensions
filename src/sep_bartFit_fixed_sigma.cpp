//
//  sep_bartFit_fixed_sigma.cpp
//  Run separate BART fits but allow the user to supply the variance.
//
//  Created by Sameer Deshpande on 20 March 2019.
//
...
// [[Rcpp::export]]
List sep_bartFit_fixed_sigma(arma::mat Y,
                             arma::mat X,
                             arma::mat X_pred,
                             arma::vec sigma_init, // user specified residual standard deviations
                             List xinfo_list,
                             int burn = 250, int nd = 1000, int m = 200, double kappa = 2, double nu = 3, double var_prob = 0.9)
{
  ...
  // Because we are allowing sigma to be fixed by the user, we need to specify the elements in the precision matrix Omega and the covariance matrix Sigma
  // Remember that Omega and Sigma are for the standardized data
  arma::mat Omega = arma::zeros<arma::mat>(q,q);
  arma::mat Sigma = arma::zeros<arma::mat>(q,q);
  for(size_t k = 0; k < q; k++){
    Sigma(k,k) = sigma_init(k) * sigma_init(k)/(y_col_sd(k) * y_col_sd(k));
    Omega(k,k) = y_col_sd(k) * y_col_sd(k)/(sigma_init(k) * sigma_init(k));
  }
  ...
  for(int k = 0; k < q; k++){
    ...
    pi.lambda[k] = (pi.sigma_hat[k] * pi.sigma_hat[k] * chisq_quantile)/nu; // This is never going to be used because we have a fixed sigma
  ...
  for(int iter = 0; iter < (nd + burn); iter++){
    ...
    // Because sigma is fixed, we don't need to carry out the Omega update.
    /*
